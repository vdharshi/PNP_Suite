...
import Global_Var
import time
print("Executing display_006")
print("Test Case ID display_006: Display DC5 support: check whether display  enter DC5 state  when display is On")
...
if reg_val1 > reg_val:
    print('SUCCESS')
    data = ["display","display_006","SUCCESS","Display enters DC5 state  when display is On"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
else:
    print('FAILURE')
    data = ["display","display_006","FAILURE","Display is not entering  DC5 state  when display is On"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
