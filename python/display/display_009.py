...
import Global_Var
import time
print("Executing display_009")
print("Test Case ID display_009: Display DC5 support: Display should enter DC5 state when display is Off")
time.sleep(10)
...
print("AFTER DISPLAY OFF: saving the DC5 value in varaible reg_val1")
with open("/sys/kernel/debug/dri/0/i915_dmc_info",'r') as f:
 index=0
 for line in f:
  index=index+1
...
if reg_val1 > reg_val:
    print('SUCCESS')
    data = ["display","display_009","SUCCESS","Display enters DC6 state when display is Off"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
else:
    print('FAILURE')
    data = ["display","display_009","FAILURE","Display is not entering DC6 state when display is Off"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
