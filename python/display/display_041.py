...
import time
import Global_Var
print("Test Case ID display_041: Display DC6 support: Display should enter DC6 state when display is Off")
time.sleep(10)
...
if reg_val1[:-1] >reg_val[:-1]:
  print("SUCCESS")
  data = ["display","display_041","SUCCESS","Display enters DC6 state when display is Off"]
  with open(Global_Var.result_path/'A.csv', 'a') as f:
    writer = csv.writer(f)
    writer.writerow(data)
else:
  print("FAILURE")
  data = ["display","display_041","FAILURE","Display is not entering DC6 state when display is Off"]
  with open(Global_Var.result_path/'A.csv', 'a') as f:
    writer = csv.writer(f)
    writer.writerow(data) 