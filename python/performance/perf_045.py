...
import time
import Global_Var
print("Test Case ID perf_045: During stress ng test, check VR TDC Throttling due to CPU")
...
if cpu_limit==0:
    print('SUCCESS')
    data = ["performance","perf_045","SUCCESS","During stress ng test No VR TDC Throttling due to CPU"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
else:
    print('FAILURE')
    data = ["performance","perf_045","FAILURE","During stress ng test VR TDC Throttling due to CPU"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
