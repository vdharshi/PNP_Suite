...
import time
import Global_Var
print("Test Case ID perf_043: During stress ng test, check RATL Throttling due to CPU")
...
if cpu_limit==0:
    print('SUCCESS')
    data = ["performance","perf_043","SUCCESS","During stress ng test No RATL Throttling due to CPU"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
else:
    print('FAILURE')
    data = ["performance","perf_043","FAILURE","During stress ng test RATL Throttling due to CPU"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
