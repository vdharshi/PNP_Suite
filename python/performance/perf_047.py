...
import time
import Global_Var
print("Test Case ID perf_047: During stress ng test, check PL1 Throttling due to CPU")
...
if cpu_limit==0:
    print('SUCCESS')
    data = ["performance","perf_047","SUCCESS","During stress ng test No PL1 Throttling due to CPU"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
else:
    print('FAILURE')
    data = ["performance","perf_047","FAILURE","During stress ng test PL1 Throttling due to CPU"]
    with open(Global_Var.result_path/'A.csv', 'a') as f:
     writer = csv.writer(f)
     writer.writerow(data)
